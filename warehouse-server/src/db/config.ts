    ...
    try {
        ...
        await client.query(`
            CREATE TABLE IF NOT EXISTS box_events (
                ...
                event_type     VARCHAR(30) NOT NULL CHECK (event_type IN ('archived','retrieved','empty_carton_issued','disposed')),
        ...
        // Widen the event_type CHECK to include 'disposed'. Additive only —
        // safe in any order relative to existing rows.
        await client.query(`
            ALTER TABLE box_events DROP CONSTRAINT IF EXISTS box_events_event_type_check
        `);
        await client.query(`
            ALTER TABLE box_events ADD CONSTRAINT box_events_event_type_check
                CHECK (event_type IN ('archived','retrieved','empty_carton_issued','disposed'))
        `);
        ...
        await client.query(`
            CREATE UNIQUE INDEX IF NOT EXISTS idx_payroll_unique_original
                ON payroll(staff_id, month, year) WHERE reverses_payroll_id IS NULL
        `);
